...
app.get('/createpile/:adminemail', (req, res) => {
    ...
    con.connect(function (err) {
        ...
        con.query(
            "INSERT INTO `simple_piles` (`pile_id`, `admin_email`, `data`) VALUES (NULL, \'"+req.params.adminemail+"\', \'"+JSON.stringify(defaultData)+"\');",
            function (err, result) {
            if (err) throw err;
            var_dump(result[0]);
            con.query(
...
app.get('/adduser/:pileID/:userEmail/:sumToPay', (req, res) => {
    ...
    con.connect(function (err) {
        ...
        var data;
        con.query(
            "SELECT * FROM `simple_piles` WHERE `pile_id` = \'"+req.params.pileID+"\';",
            function (err, result) {
            if (err) throw err;
            var_dump(result[0]);
            ...
            con.query(
                "UPDATE `simple_piles` SET `data` = \'"+JSON.stringify(data)+"\' WHERE `simple_piles`.`pile_id` = \'"+req.params.pileID+"\';",
                function (err, result) {
                if (err) throw err;
                var_dump(data);
                var_dump(result[0]);
